...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning("Background worker no terminó en tiempo esperado")
        app.state.background_task.cancel()

    # Esperar workers de Telegram con timeout usando asyncio.wait
    ...
    if pending:
        logger.warning(f"Cancelando {len(pending)} workers de Telegram que no terminaron")
        for task in pending:
            task.cancel()
        # Esperar a que se cancelen limpiamente
...
class OptimizedIPBlockMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        excluded_paths = {"/docs", "/redoc", "/openapi.json", "/health", "/metrics", "/login","/guardar_datos","/ver_datos" }
